...
### 2. output 작성
백엔드가 활성화되면 Terraform은 명령을 실행하기 전에 S3 버킷에서 최신 상태를 자동으로 가져오고 명령을 실행한 후에는 최신 상태를 S3 버킷으로 자동으로 푸시.
이를 확인하기 위해 출력 변수 작성
...
Terraform은 S3에 상태 데이터를 자동으로 푸시하거나 가져오고 S3기 싱테 파일의 모든 변경 사항을 저장  
